# -*- coding: utf-8 -*-
"""高雄市立林園高級中學_131051_黃紹銓

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1sx_B35furzAD4YVwzYB_dacniDdrz-xC
"""

import seaborn as sns
import warnings
warnings.filterwarnings("ignore", category=FutureWarning)
import pandas as pd
import matplotlib.pyplot as plt

# 讀取 CSV 檔案
df = pd.read_csv('觀光遊憩據點按縣市及遊憩據點交叉分析.csv')

# 定義縣市名稱映射
county_name_mapping = {
    '金門縣': 'Kinmen',
    '連江縣': "Lianjiang",
    '澎湖縣': 'Penghu',
    '臺東縣': 'Taitung',
}

# 將縣市中文名稱替換為英文名稱
df['縣市'] = df['縣市'].map(county_name_mapping)

# 選擇月份欄位並計算總和
月份欄位 = df.columns[4:-1]

# 將月份欄位轉換為數值類型，處理日期格式
for 欄位 in 月份欄位:
    # 提取數字並轉換為數值，無法轉換的設為 NaN
    df[欄位] = pd.to_numeric(df[欄位].str.extract(r'(\d+)', expand=False), errors='coerce')

# 計算每行的總和
df['總和'] = df[月份欄位].sum(axis=1)

# 按照縣市分組並計算中位數和最大值
縣市總和 = df.groupby('縣市')['總和'].agg(['median', 'max']).reset_index()

# 使用 seaborn 生成箱型圖
plt.figure(figsize=(12, 6))  # 設定圖形大小
sns.boxplot(x='縣市', y='總和', data=df, order=list(county_name_mapping.values()), showfliers=False)

# 定義 stats 變數，將縣市作為索引
stats = 縣市總和.set_index('縣市')

#箱形圖算中位數、最大數的語法
for i, county in enumerate(stats.index):
    plt.text(i, stats['median'][county], f'Median: {int(stats["median"][county])}',
             ha='center', va='bottom', fontsize=10, color='black')
    plt.text(i, stats['max'][county], f'Max: {int(stats["max"][county])}',
             ha='center', va='bottom', fontsize=10, color='blue')

# 添加標籤和網格
plt.xlabel('County', fontsize=14)  # x 軸標籤
plt.ylabel('Tourists', fontsize=14)  # y 軸標籤
plt.grid(axis='y')  # 顯示 y 軸網格
plt.xticks(rotation=45)  # 旋轉 x 軸標籤以避免重疊
plt.tight_layout()  # 自動調整佈局以避免重疊

# 儲存圖表為 JPG 檔案
plt.savefig('001.jpg')
plt.clf()  # 清空圖形

import json
import pandas as pd

with open('HotelList.json', 'r', encoding='utf-8-sig') as file:
    data = json.load(file)

hotels_df = pd.json_normalize(data['Hotels'])

# 詢問篩選條件
user_input_city = input()

# 1. 檢查是否有符合地點的旅館
matching_hotels_by_city = hotels_df[hotels_df['PostalAddress.City'] == user_input_city]
if matching_hotels_by_city.empty:
    print("未找到符合地點的旅館/民宿")
else:
    user_input_service = input()

    # 篩選符合特殊需求的旅館，若跳過則使用原始結果
    matching_hotels = matching_hotels_by_city
    if user_input_service.lower() != '無' and user_input_service != '':
        matching_hotels = matching_hotels_by_city[
            matching_hotels_by_city['ServiceInfo'].str.contains(user_input_service,case=False, na=False)]
        from tabulate import tabulate
        if matching_hotels.empty:
            print("未找到符合地點和特殊需求的旅館/民宿")

    # 若前面篩選有結果，則繼續詢問預算
    if not matching_hotels.empty:
        user_input_budget = int(input())

        # 篩選符合預算的旅館
        matching_hotels_final = matching_hotels[
            (matching_hotels['LowestPrice'] <= user_input_budget) &
            (matching_hotels['CeilingPrice'] >= user_input_budget)
        ]
        if matching_hotels_final.empty:
            print("未找到符合預算的旅館/民宿")
        else:

            print(matching_hotels_final[['HotelName', 'LowestPrice', 'CeilingPrice', 'Description', 'PostalAddress.Town', 'PostalAddress.StreetAddress']].head(5).to_string(index=False))